...
; get sine
	ld hl, SineTableSize
	ld de, SineOffset	; input
	call GetSine
	ld de, ScrollVar1	; destination
	call GetSine2
	
	ld hl, SineTable2Size
	ld de, Sine2Offset	; input
	call GetSine
	ld de, ScrollVar3	; destination
	call GetSine2
	sub a, 15
...
; update scx
	...
	ret
.finishtext
	ld hl, ScrollerText		; reset scroller text address
	ld a, h
	ld [CurrentTextPointer], a
	ld a, l
	ld [CurrentTextPointer+1], a
...
ScrollerText:
	db "Here it is. An intro/demo thingy. "
	db "This is my second demo which is still not that good, but snort."
	db " Also screen flickering that I'm too lazy to fix, blablabla, "
	db "no sprites, just a lame text scroller with a logo at the top. scan"
	db "line tricks, some familiar music, yeah. Sound engine made"
	db " by me (a pretty basic one), graphics made with a generator, and the font is from GBDK."
	db "                             @"
